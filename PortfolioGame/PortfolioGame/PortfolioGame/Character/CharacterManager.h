...
#include "../Component/Collision/CollisionSystem.h"
...
public:
	//コンストラクタ
	CharacterManager(MovementSystem& movement_system_, CollisionSystem& collision_system_ ,AnimationSystem& animation_system_, RenderingSystem& rendering_system_) :
		movement_system(movement_system_),
		collision_system(collision_system_),
...
private:
	...
	//キャラクターのコンポーネント登録
	void RegisterComponent(CharacterBase& character_);
...
private:
	...
	//生存できる敵の上限数
	int can_arrive_enemy_limit{ 10 };

	//敵の生成間隔
	float enemy_spawn_interval{ 10.0f };
