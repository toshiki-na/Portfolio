...
//float比較用微小値
constexpr float QUATERNION_EPSILON = 1.0e-6f;
...
public:
	...
	inline static Quaternion RotateAngleAndAxisToQuaternion(float angle_, const Vec3& axis_)
	{
		...
		if (std::abs(axis.LengthSq() - 1.0f) > QUATERNION_EPSILON)
		{
	...
	inline static Quaternion Slerp(const Quaternion& start_, const Quaternion& end_, float rate_)
	{
		...
		if (std::abs(start.LengthSq() - 1.0f) > QUATERNION_EPSILON)
		{
			start = start.Normalized();
		}
		if (std::abs(end.LengthSq() - 1.0f) > QUATERNION_EPSILON)
		{
			end = end.Normalized();
		}
